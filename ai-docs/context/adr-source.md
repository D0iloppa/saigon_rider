...
- 푸시는 앱이 꺼져 있을 때를 커버한다 — 켜져 있을 때는 폴링이 맡는 2층 구조. 체감 지연은 최대 `dmPollInterval`(기본 30초), 포그라운드 복귀 시엔 즉시. **주기는 구 어드민(`/admin-legacy/`)에서 재빌드 없이 바꾼다**(설정 키 `dm.unread_poll_interval`, `app_version.py:24`).
- **폴링 실태(2026-09-13 전수조사)**: 홈 유휴 시 도는 폴러는 뱃지 1개뿐이고 1분당 약 4건 — 부하는 문제가 아니었다. 실제 낭비는 백그라운드에서도 도는 폴러들이었고 `pollScheduler` 로 정리했다. 스케줄러를 **일부러 안 쓰는 예외**는 `RideNav` trail·`useRideStore` active-card(3s) 둘뿐 — 길안내·퀘스트 추적이라 화면이 꺼져도 돌아야 한다.
...
- 읽음 데이터는 이미 존재 — `dm_messages.read_at`(1:1), `dm_conversation_members.last_read_at`(멤버별 워터마크), `POST /conversations/{id}/read`. 1:1 버블은 체크 아이콘 표시(`DmDetail.tsx:1196-1207`). **그룹의 메시지당 안읽은 인원수만 미구현**(1:1 은 "읽음" 텍스트, 그룹은 숫자 — 대표 택1 2026-09-13).
...
- **DM 을 폴링으로 둔 대가**: SSE 인프라가 이미 있는데도 DM 은 폴링이다. 구현은 단순하지만 즉각성이 없고(뱃지 최대 30초) 서버 부하가 상시 발생한다.
...
- DM 뱃지 폴링 주기를 30초에서 낮출지(10초 권고). 어드민 설정값이라 코드 변경 불요.
...
- 그룹 메시지 읽음 인원수 미구현 (티켓 `2026-09-13-dm-notify-realtime-readreceipt` P3)
- iOS·안드로이드 푸시 모두 실기기 수신 확인됨(2026-09-13). DLQ(`noti:events:dlq`) 에 장애 기간 실패분 62건 잔존.
...
- **증거 없는 완료 없음** — 정적 코드 재확인은 Verify 를 바꾸지 못한다. 실행 일시·명령·관찰값·증거 경로가 있어야 PASS 다. 코드 검사만으로 원인을 단정하지 말고 런타임 증거(로그·`pg_stat_activity`·실제 발송 결과)를 확보한다 — 2026-09-13 푸시 장애는 코드상 정상으로 보이는 경로가 런타임에 죽어 있던 사례이며, APNs 환경 불일치라는 그럴듯한 가설이 실측으로 기각됐다. 성능 개선도 같다 — "폴링이 많아 보인다"는 인상 대신 전수조사로 홈 유휴 4건/분을 확인하고 나서야 진짜 낭비 지점(백그라운드 미가드)이 드러났다.
