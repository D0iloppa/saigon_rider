...
- **내 위치 마커는 "위에서 본 오토바이"이고, 회전 기준은 `rotationAlignment: 'map'` 이다**(W18, 2026-08-07). 종전에는 파란 원을 `heading` 만큼 돌리고 있어서 — 원은 회전해도 변화가 없으므로 — **방향 표시가 사실상 없었다.** 아이콘은 `components/ride/riderMarkerIcon.ts`(30px, DOM 직접 삽입용 SVG 문자열). ① 회전은 엘리먼트 transform 이 아니라 **`Marker.setRotation(진북 기준 heading)`** 으로 건다 — 마커를 `rotationAlignment: 'map'` 으로 만들면 MapLibre 가 `rotateZ(rotation - bearing)` 을 대신 계산하므로 북향·course-up·수동 회전 어디서나 한 값으로 맞고, `heading - bearing` 을 우리가 프레임마다 다시 넣을 필요가 없다(뷰포트 기본값 그대로 `heading` 을 주면 course-up 에서 **회전이 두 배**가 된다). ② `heading` 이 없는 틱(정지·GPS 결측·NaN)에는 **직전 유효 heading 을 유지**한다 — 0 으로 되돌리면 멈출 때마다 북쪽으로 홱 돈다. ③ **첫 유효 heading 이 오기 전에는 오토바이를 띄우지 않고 기존 파란 dot 을 유지**한다 — 방향을 모르는데 북쪽을 향해 세워두면 "정보 없음"이 아니라 거짓 정보다. 계약: `components/ride/rideNavRiderMarker.contract.test.mjs`. 동네지도(`SaigonMapV5`)의 내 위치 점·heading 삼각형은 **탐색용이라 역할이 달라 이 규칙 밖**이다(대표 결정).
...
| `WITHDRAWN` (철회) | ✘ | **판매자 본인 조회 시 예외 — SOLD 와 동일** (개정 2026-08-08, 대표 지시) |
| `HIDDEN` / `REMOVED` | ✘ | 모더레이션, **판매자 본인에게도 비노출** |

- `SOLD` 전이는 `market.py::complete_appointment` 한 곳에서만 일어나고 **종결 상태**다(SOLD → 다른 상태 전이 전부 거부).
- **`WITHDRAWN` 은 종결 상태가 아니다 (개정 2026-08-08, 대표 지시 "매물철회를 하면 리스트에서 빠지면 안 된다. 상태로 관리해야지 / 다시 팔 수 있게").** 철회는 삭제가 아니라 되돌릴 수 있는 상태다:
  - 내 매물 목록·상세에 계속 남는다(판매자 본인 한정). 재판매 진입점이 이 목록이므로 여기서 사라지면 매물이 영구 증발한다.
  - `WITHDRAWN → ON_SALE` 전이 허용. 구매자가 없는 상태의 `RESERVED` 직행만 거부(400 `relist_on_sale_only`).
  - 철회 상태에서 **본문·가격 수정 허용** — "잠시 내렸다가 고쳐서 다시 올리는" 흐름이 정책이다.
  - 재판매 시 `bumped_at` 은 갱신하지 않는다 — 철회↔재판매 반복이 공짜 끌올이 되면 안 된다.
  - ⚠️ 모더레이션(`HIDDEN`/`REMOVED`)과 한 묶음으로 다루지 말 것. 되돌릴 수 없는 것은 모더레이션과 `SOLD` 뿐이다.
...
1. **초대 자격 = 초대자의 팔로잉.** 그룹 개설·멤버 초대 모두 **초대하는 사람이 대상을 팔로우 중**이어야 한다(`backend/app/services/dm_policy.py::require_invite_eligible`). 맞팔(친구)은 팔로잉의 부분집합이라 함께 통과한다. **이 판정은 서버가 SoT** — 종전엔 클라이언트만 맞팔 목록을 노출했을 뿐 서버는 관계를 전혀 검증하지 않아 API 로 아무나 밀어넣을 수 있었다(2026-08-28 이전 상태).
   - **단, 자격은 '처음 들어오는 사람'에게만 요구한다.** 한 번 멤버였던 사람(강퇴·자진 탈퇴로 `left_at` 이 찍힌 경우)의 재초대에는 팔로우 여부를 묻지 않는다 — 요구하면 남은 운영진 중 그를 팔로우하는 사람이 없을 때 4항의 "강퇴는 재초대로 복귀 가능"이 성립하지 않는다. 이미 활성 멤버인 id 는 검사 없이 건너뛴다(배치 초대가 통째로 403 되지 않도록).
...
4. **강퇴와 블랙리스트는 다른 제재다.**
   ...
   - **블랙리스트**(`dm_conversation_bans`, 212 마이그레이션): 해제 전까지 **초대·입장 모두 거부**(`require_not_banned`). 오픈톡방 입장에도 적용되어, 커뮤니티 그룹 멤버라도 방 단위로 밴됐으면 못 들어온다. **적용 지점이 두 곳이다** — `dm.py::join_open_conversation` 과 `community_groups.py::_add_open_conversation_member`(그룹 가입·승인 시 오픈톡방 멤버를 되살리는 경로). 후자를 빠뜨리면 밴당한 사용자가 **커뮤니티 그룹을 탈퇴했다 재가입하는 것만으로 방에 복귀**한다(2026-08-28 리뷰에서 발견).
...
## 워키토키 재생·백그라운드 (제정 2026-08-28)

1. **자동재생은 "활성 채널"에만.** 사용자가 명시적으로 참여한 채널(캡슐이 활성화 → `syncActiveChannel` 이 SharedPreferences 에 기록)의 음성만 자동으로 소리가 난다. 다른 대화방의 음성은 **알림만** 뜨고, 사용자가 "재생"을 눌러야 들린다. 무전기라도 아무 방의 소리가 갑자기 튀어나오면 안 된다.
...
| | Android | iOS |
|---|---|---|
| 백그라운드 녹음·발송 | ✅ 오버레이/위젯 → 헤드리스 큐 → 앱 실행 시 업로드 | ❌ 헤드리스 녹음 경로 없음 |
| 백그라운드 알림 | ✅ | ✅ |
| 백그라운드 자동재생 | ✅ (본 절) | ❌ **플랫폼 정책상 불가** — 사용자 상호작용 없는 오디오 재생이 막혀 있어 알림 + 탭 재생이 최선 |
...
7-1. **음성 푸시는 채널에 접속 중이지 않은 수신자에게만 간다 (2026-08-29).** 모듈 `service.send_voice` 가 `present()`(presence TTL 45초) 에 있는 수신자를 제외한다 — 접속 중이면 인앱 자동재생(위 8항)이 이미 소리를 내므로 푸시는 같은 음성을 두 번 알리는 꼴. 이 규칙이 성립하려면 클라이언트가 **백그라운드 전환 시 즉시 `leave`** 해야 한다(캡슐 `visibilitychange` 핸들러) — TTL 만 믿으면 백그라운드 직후 45초간 음성이 조용히 유실된다. 음성 푸시를 통째로 끄면 Android 백그라운드 자동재생(4항)이 죽으므로 끄지 않는다.
7-2. **iOS 푸시 전제 = Firebase 콘솔 APNs 인증키.** FCM 이 `401 THIRD_PARTY_AUTH_ERROR "Invalid APNs credential"` 을 돌려주면 코드 문제가 아니라 콘솔(프로젝트 설정 → Cloud Messaging → Apple 앱 → APNs 인증키 .p8 + Key ID + Team ID) 문제다. engine 은 이 오류를 **영구 실패**로 분류한다(재시도 5회·DLQ 적재 금지 — 2026-08-28 실측 127건).
...
11. **캡슐의 X 는 "숨김"이 아니라 "채널 이탈"이다 (2026-09-10 대표 지시 — 2026-08-27 지시 번복).** 종전엔 X 가 비영속 `closed` 플래그만 세웠고 `activeConversationId`/`activeConversationMeta` 는 persist 되어, **앱을 다시 열면 캡슐이 되살아났다**(당시엔 그게 의도였다 — "재기동 시 버블은 기본적으로 다시 나타나는 게 요구사항"). 대표 실기기 제보로 번복: 이제 `close()` 가 persist 되는 두 필드를 `null` 로 지우고 `native.walkieTalkie.syncActiveChannel({channelId:'',channelName:''})` 로 **Android 채널 바로가기 위젯까지 해제**한다. 확인 다이얼로그도 없다(재참여가 1탭이라 마찰만 늘린다).
    - **재진입 경로 (X 이후 실제로 도달 가능한 것만).** "캡슐 컨텍스트메뉴 '채널 변경'"은 캡슐이 렌더될 때만 존재하는데 `WalkieTalkieFloatingButton` 이 `if (!bubbleActive) return null`로 끝나 X 이후엔 캡슐 자체가 없다 — **이탈 후엔 도달 불가**, 켜져 있는 채널을 다른 채널로 *바꾸는* 용도로만 쓰인다. 실제 재진입은: (a) **헤더메뉴 "워키토키"** — 홈/DM목록 헤더의 `WalkieTalkieEntryButton`(최근 대화 선택 시트 → `joinWalkieChannel`) 또는 DmDetail 자체 더보기메뉴의 워키토키 버튼(현재 보고 있는 방으로 바로 참여); (b) **초대카드 "참여하기"** — 그 대화를 다시 열어야 카드가 보인다; (c) **음성알림 딥링크(`?voice=1`)** — 아래 항목.
    - **음성알림 딥링크는 진입당 1회만 활성화한다.** `DmDetail` 의 `voiceJoinedForRef`(conversationId 값을 기억)가 이를 보장 — 🔒 이 effect 의 deps 에 `walkieActiveConversationId` 를 절대 넣지 마라. 넣으면 X 로 스토어가 비워질 때 이 effect 가 재실행되어 즉시 재활성화시키므로, `?voice=1` 화면에서 X 가 무력화된다(회귀 재현: 음성알림 탭 → 진입 → 캡슐 X → 캡슐이 도로 나타남).
    - 이 변경으로 `closed` 필드·`open()` 액션·`bubbleActive` 의 `!closed` 절이 **도달 불가가 되어 제거**됐다. 되살리지 마라 — `bubbleActive` 는 이미 `!!conversationId` 를 요구하므로 대상 대화를 지우는 것만으로 캡슐이 사라진다. "숨김 상태"를 다시 도입하면 X 의 의미가 두 갈래로 갈린다.
    - **서버 이탈 신호는 자동이다** — presence effect 가 `[conversationId, bubbleActive]` 에 걸려 있어 대상 대화가 `null` 이 되면 cleanup 이 `walkieApi.leave()` 를 호출한다(상대 화면의 유령 참석자 방지). 별도 배선을 추가하지 마라.
    - 로그아웃 경로(`reset()`)는 **X(`close()`)와 결과가 같다** — 위젯 활성 채널도 함께 해제한다(다음 사용자에게 이전 계정 채널이 남지 않도록, `MyFirebaseMessagingService.maybeAutoPlay` 의 백그라운드 자동재생 게이트이기도 하다). 회귀 고정은 `frontend/src/store/useWalkieTalkieBubbleStore.contract.test.mjs`.
    - **알려진 갭 (미해결, 빌드 금지):** 헤드리스 녹음 드레인(6항, `WalkieTalkieFloatingButton` 의 pending 업로드 루프)은 `bubbleActive` 게이트라 X 이후엔 그 채널 몫이 다시 돌지 않는다. 오늘은 실질 프로듀서가 위젯 탭 경로(`WalkieTalkieWidgetProvider`, channelId="" 로 적재되어 버려짐)뿐이라 잠재적일 뿐 — 하지만 오버레이 버블 경로(`WalkieTalkieOverlayService`, 실제 channelId 로 녹음)가 JS 에서 실제로 기동되게 배선되는 순간 라이브가 된다. 그 배선을 하는 사람은 드레인 트리거를 `bubbleActive` 외의 조건(예: 앱 포그라운드 복귀)으로도 열어야 한다.
