...
백엔드 `backend/app/routers/admin_api/feed.py`(`GET /admin/api/community/feed[/:id]`, `DELETE /community/feed/{id}`), `verify_admin_api`(레거시 `/admin-legacy/feed`와 동일 admin 레벨) + 삭제 시 `_audit.audit()`(`FEED_DELETE`). 삭제는 DB `ON DELETE CASCADE`로 좋아요/댓글/이미지 동반 삭제. 이미지는 앱 라우터의 `_resolve_image_urls`(contents 중개+imgproxy) 재사용. 좌표 Decimal→문자열은 프론트 `Number()` 강제. fetch 훅 `admin-frontend/src/api/feed.ts`. **작성/편집(authoring) 이식 완료 (2026-07-23, 커밋 `b0e3f5e`)**: `POST /admin/api/community/feed`(create)·`PUT .../{id}`(update), author=공용계정(`ADMIN_USER_ID`), 필드 content·is_story(STORY)·image_content_id[](FeedPostImage fanout). 화면 `pages/community/FeedEditPage.tsx`(라우트 `/community/feed/new`·`/community/feed/:id/edit`, FeedList "새 피드 작성"·FeedDetail "수정" 버튼 — 별도 메뉴 아닌 서브액션, 레거시 동일). 이미지는 업로드 위젯 대신 `content_id` UUID 입력(POI/badge 관례, 줄바꿈 구분·첫 줄=커버). 감사 `FEED_CREATE/UPDATE`. **알려진 갭**: 레거시의 멀티파트 직접 업로드+이미지 개별삭제 체크박스는 미이식(의도 — content_id 방식). 신고센터는 아직 피드 미포함(`Report.target_type`에 FEED 추가는 별도 백로그).
