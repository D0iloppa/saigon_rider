...
이 문서 체계에서 **“스토리보드”는 `ai-docs/review/storyboard/index.html`을 의미한다.** 서비스 전체 플로우의 와이어프레임, 화면·상태·상호작용, `F-… / FR-…` 프레임 ID를 구현·검수·보고서의 UI/UX 기준으로 삼는다. 구현자는 이 파일을 확인한 뒤 작업하고, 승인된 동작·레이아웃·상태를 추가하거나 변경하면 코드와 함께 스토리보드도 갱신한다. 스토리보드와 구현이 다르면 구현을 완료로 판정하지 않는다.

스토리보드 HTML은 `ai-docs/review/storyboard/build.mjs`로 재생성할 수 있다. 생성 원본을 수정하는 작업은 재생성된 `index.html`까지 함께 반영해 원본·산출물의 불일치를 방지한다.
