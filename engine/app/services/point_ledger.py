...
async def get_or_create_balance(db: AsyncSession, user_id: int) -> RpBalance:
    balance = await db.get(RpBalance, user_id)
    if balance is None:
        balance = RpBalance(user_id=user_id)
        db.add(balance)
        await db.flush()
    return balance
...
async def lock_balance(db: AsyncSession, user_id: int) -> RpBalance:
    result = await db.execute(
...
def round_rp(value: Decimal) -> int:
...
async def credit(
    ...
    *,
    ...
    # 만료 스케줄 생성 (적립일 + 3개월)
    expires_at = now + relativedelta(months=settings.sre_rp_expiry_months)
    schedule = RpExpirationSchedule(
        user_id=user_id,
    ...
    *,
    ...
    if balance.current_balance < amount:
        raise InsufficientBalanceError(
            f"Required {amount} RP but current balance is {balance.current_balance}"
        )
...
async def _consume_expiration_fifo(
    ...
    schedules = (
        await db.execute(
            ...
            .order_by(RpExpirationSchedule.expires_at.asc(), RpExpirationSchedule.expire_id.asc())
            .with_for_update()
        )
    ).scalars().all()
...
async def get_daily_earned(
    ...
    result = await db.execute(
        select(func.coalesce(func.sum(RpTransaction.amount), 0)).where(
            RpTransaction.user_id == user_id,
            RpTransaction.tx_type == TxTypeEnum.EARN,
            RpTransaction.occurred_at >= day_start,
            RpTransaction.occurred_at <= day_end,
        )
    )
