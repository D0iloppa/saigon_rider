...
from ..services.service_area import locate_ward_slug
...
# HCMC 서비스 지역 bbox — 동네지도 쪽 HCMC_BBOX(frontend NeighborhoodMap.tsx 등)와 동일 값.
# service_area.py 의 geometry(depth1.json)는 지도 렌더링용 도심 코어 폴리곤만 담아 Thủ Đức 등
# 외곽 구가 빠져 있어 여기엔 부적합 — 실제 서비스 지역 전체를 덮는 이 값을 쓴다.
_HCMC_BBOX = {"s": 10.40, "n": 11.10, "w": 106.40, "e": 107.00}
...
async def get_map_data(
    ...
    # 반경 검색(radius_km)만으로는 강 건너 인접 성 등 서비스 밖 좌표까지 잡혀 지도에 엉뚱한 핀이
    # 뜬다(예: 오탐/시드 데이터) — HCMC 서비스 지역 bbox 로 한 번 더 한정한다.
    #
    # 2026-07-31: 세 쿼리의 LIMIT 을 50 → 1000 으로 올렸다. 침수지도는 "도시 전역 전량"이
    # 요구사항인데, 핫스팟(flood_hotspot_stats — 제보 누적에서 파생)이 50개를 넘어가면
    # 상위 50개만 내려가 지도가 **아무 표시 없이 조용히 잘렸다**. dev DB 는 핫스팟 30개라
    # 증상이 드러나지 않았을 뿐이고 운영에서는 터질 문제였다. bbox+radius_km 로 이미
    # 경계가 있으므로 1000 은 절단선이 아니라 폭주 방지용 안전판이다.
    # 주의: 이 응답 하나를 지도 마커와 하단 리스트가 공유한다 — 리스트를 페이지네이션하려면
    # 지도용/리스트용 조회를 먼저 분리해야 한다(그러지 않으면 페이징이 곧 지도 절단).
    bbox_params = {
        "bbox_s": _HCMC_BBOX["s"],
        "bbox_n": _HCMC_BBOX["n"],
        "bbox_w": _HCMC_BBOX["w"],
    ...
    reports_result = await db.execute(
        text("""
            ...
            WHERE fr.status = 'ACTIVE'
              AND fr.expires_at > NOW()
              AND fr.lat BETWEEN :bbox_s AND :bbox_n
              AND fr.lng BETWEEN :bbox_w AND :bbox_e
              AND ST_DWithin(
                    fr.geom,
                    ST_SetSRID(ST_MakePoint(:lng, :lat), 4326)::geography,
                    :radius_m
    ...
    for row in reports_result:
        ...
        # district_code(레거시 구 단위)는 표시용 그대로 두고, 지도 배지 위치는 실제 렌더링 폴리곤
        # (depth1.json) 기준 ward_slug로 별도 산출 — 매칭 폴리곤 밖이면 None(배지 미노출 대상).
        r["ward_slug"] = locate_ward_slug(r["lat"], r["lng"])
        reports.append(r)
    ...
    for row in hotspots_result:
        ...
        h["ward_slug"] = (
            locate_ward_slug(h["centroid_lat"], h["centroid_lng"]) if h.get("centroid_lat") is not None else None
        )
        hotspots.append(h)
    ...
    risks_result = await db.execute(
        text("""
            SELECT risk_id, hotspot_id, district_code, street_name,
                   ...
                   rain_prob, risk_level, depth_hint, predicted_date, is_stale
            FROM flood_risk_daily
            WHERE expires_at > NOW()
              AND lat BETWEEN :bbox_s AND :bbox_n
              AND lng BETWEEN :bbox_w AND :bbox_e
              AND ST_DWithin(
                    geom,
                    ST_SetSRID(ST_MakePoint(:lng, :lat), 4326)::geography,
                    :radius_m
