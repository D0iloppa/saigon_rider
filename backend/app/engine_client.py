...
class EngineClient:
    ...
    async def admin_stream_dlq_messages(self, *, count: int = 50) -> list[dict]:
        """F-21: Engine worker DLQ(격리 메시지) 조회. Engine DB 미개입 — Redis Stream 프록시일 뿐."""
        resp = await self._client.get("/v1/admin/stream/dlq-messages", params={"count": count})
        resp.raise_for_status()
        return resp.json()
