...
| 2026-06-07 | F-03-IME | PROFILE-SETUP · 닉네임 한글 밀림+[시작하기] 미활성 (Android) | Android 회원가입 시 한글 한 글자씩 밀림 + 입력해도 [시작하기] 안 켜짐. 블로커. iOS·PC 정상 | Android WebView: input 이 **타이핑 중 React 리렌더되면 한글 조합 버퍼를 끊음**. 직전 uncontrolled+input리스너 버전도 입력마다 setState→리렌더라 미해결 | **타이핑 중 setState 완전 제거**: Android 는 uncontrolled+ref, 제출 시 ref.value 만 읽음(React 타이핑 미개입). 버튼은 스타일 선택 시 활성, 닉네임 검증은 제출 시. iOS/PC 기존 controlled. **web 수정(빌드 불필요)**. 보조로 native adjustPan(android `2b7ce8d`) | 🔧 |
...
| 2026-06-07 | F-03-KBD | PROFILE-SETUP · 닉네임 입력창/카드 높이 줄어듦 (Android) | Android 키보드 표시 시 입력창·카드 높이 압축. 아이폰 정상 | 뷰포트 축소 시 `.nickField`(60px)·`.styleCard` 가 flex 자식 기본 `flex-shrink:1` 로 눌림(키보드 모드 무관 CSS) | **`flex-shrink:0`** 부여(ProfileSetup.module.css) → 압축 대신 body 스크롤. **web 수정(빌드 불필요)**. 보조 native adjustPan + 정적 interactive-widget | 🔧 |
