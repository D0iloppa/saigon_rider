...
# ── F-21: DLQ 조회 (격리는 구현돼 있었으나 조회 경로가 없었다) ──────────────


@router.get("/dlq", summary="Engine worker DLQ 격리 메시지 조회")
async def get_engine_dlq(
    count: int = Query(50, ge=1, le=500),
    _session: AdminSession = Depends(verify_root_api),
):
    """Engine DB 직접 접근 없음 — engine_client 로 Engine 의 `/v1/admin/stream/dlq-messages` 경유.

    실패 표현 규약(ADR): 조회 실패를 빈 배열로 위장하지 않는다 — items.py 의 admin_get_items 와
    동일하게 502 로 전파해 "DLQ 가 정상적으로 비어 있음"과 "Engine 연결 실패"를 구분한다.
    """
    try:
        return await engine_client.admin_stream_dlq_messages(count=count)
    except Exception as e:
        raise HTTPException(status_code=502, detail=str(e)) from e


@router.get("/noti-dlq", summary="noti_worker DLQ 격리 메시지 조회")
async def get_noti_dlq(
    count: int = Query(50, ge=1, le=500),
    _session: AdminSession = Depends(verify_root_api),
):
    """noti_worker 는 BFF 소유 프로세스(같은 Redis) — Engine 경유 불필요, 직접 조회."""
    from ...services.noti_events import STREAM_KEY

    r = await get_client()
    raw = await r.xrevrange(f"{STREAM_KEY}:dlq", count=count)
