...
- **동네지도 업체 레이어 고도화 1차 — 업종 아이콘·자동 새소식 말풍선·business_news 실데이터 (2026-07-11) 코드·시각검증 DONE·커밋됨** — SoT [`260711_map_biz_icons_autobubble_news_task.md`](../task/active/260711_map_biz_icons_autobubble_news_task.md). init/118(소식 시드 4건), `/biz/public/map` latest_news, 핀 글리프 5종+칩·카드 아이콘, 뷰포트 idle 자동 말풍선(lat 스팬 ≤0.03·중앙 반경 0.25·시트 무이동), 말풍선 정직화(소식 없으면 소개 폴백). 헤드리스 3시나리오 PASS. **잔여**: qm-reviewer 독립 리뷰(2차 패키지와 묶음), 운영 배포 시 init/118+BFF 재시작. **주의**: 같은 커밋에 이전 세션 NeighborhoodProfile WIP 훅업 헝크 혼재(선행 분리 커밋 완료, 프로필 기능 완성도는 미검증 — 대표 승인下 커밋).
- **동네지도 당근 UX 2차 (2026-07-11, 진행중)** — SoT [`260711_map_daangn_ux_task.md`](../task/active/260711_map_daangn_ux_task.md). W1 지도보기 필 / W2 핀 터치→포스트 패널(시트 대체·캐러셀·N명 보는중 Redis 폴링) / W3 카테고리 DB화·확장+소식 사진 N장 / W4 핀 unread 뱃지(localStorage). 대표 지시 운용: 분석→구현 에이전트 분리, 모델 라우팅(분석 Sonnet / W2 구현 Fable / 나머지 Sonnet). 분석 3기 완료 기준 구현 순서 W3→W1→W2→W4.
