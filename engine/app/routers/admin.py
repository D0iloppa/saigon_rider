...
@router.get("/stream/dlq-messages", dependencies=[_svc])
async def admin_stream_dlq_messages(
    count: int = Query(50, ge=1, le=500),
) -> list[dict]:
    """F-21: DLQ 격리 메시지 조회 (XREVRANGE — 최신순, 읽기 전용).

    격리 자체는 이미 구현돼 있었으나(app/workers/__main__.py) 조회 경로가 없어 쌓여도
    보이지 않았다. admin_stream_messages 와 동일한 패턴, 대상만 DLQ 스트림으로 교체.
    """
    from app.redis_client import STREAM_KEY, get_redis
