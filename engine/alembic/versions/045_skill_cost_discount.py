"""스킬 COST_DISCOUNT 합산 — pull_gacha/purchase_shop_item 에 p_skill_discount_pct 추가

BFF가 전달하는 cost_discount 스킬 할인%를 아이템 착용효과 할인과 가산한다.
가산 후 상한 50%(기존과 동일). 파라미터는 DEFAULT 0 → 기존 3-arg 호출 하위호환.
sre035 본문에서 시그니처 1줄 + 할인 가산 1줄만 변경한 DROP+CREATE.
...
revision: str = "sre045"
down_revision: Union[str, None] = "sre044"
...
BEGIN
  ...
  -- 착용효과 + 스킬 COST_DISCOUNT 가산(상한 50%)
  v_discount_pct := LEAST(_resolve_cost_discount(p_user_id) + p_skill_discount_pct, 50);
...
BEGIN
  ...
  -- 착용효과 + 스킬 COST_DISCOUNT 가산 후 상한 50%
  v_equip_discount := _resolve_cost_discount(p_user_id);
  v_discount_pct := LEAST(v_discount_pct + v_equip_discount + p_skill_discount_pct, 50);
...
def upgrade() -> None:
    # asyncpg 는 한 번의 실행에 다중 SQL문 불가 → DROP(시그니처 변경) 과 CREATE 를 분리 실행.
    bind = op.get_bind()
    bind.exec_driver_sql("DROP FUNCTION IF EXISTS pull_gacha(BIGINT, VARCHAR, BOOLEAN)")
    bind.exec_driver_sql(_PULL_GACHA)
    bind.exec_driver_sql("DROP FUNCTION IF EXISTS purchase_shop_item(BIGINT, VARCHAR, VARCHAR)")
    bind.exec_driver_sql(_PURCHASE_SHOP_ITEM)


def downgrade() -> None:
    # sre035 의 3-arg 정의로 되돌리려면 035 본문을 재실행할 것. 여기선 no-op.
    pass
