...
// ── HCMC 전역 윤곽 (침수지도 등 배경 표시용, 서비스지역 판정과 무관) ──

export interface CityOutline {
  bbox: { S: number; W: number; N: number; E: number };
  rings: [number, number][][]; // [lat, lng] 좌표 목록의 폴리곤(들)
}

let cityOutlineCache: Promise<CityOutline> | null = null;

/** 정적 데이터라 세션 내 1회만 조회하고 결과를 재사용한다. */
export function fetchCityOutline(): Promise<CityOutline> {
  if (!cityOutlineCache) {
    cityOutlineCache = api.realFetch<CityOutline>('/map/city-outline', {}, 'bff', { silent: true });
  }
