...
### 회전(나침반) — `enableFollowCompass` (제정 2026-08-06, 개정 2026-08-06 — 추종/나침반 직교 2축, 개정 2026-08-07 — 회전축 3-state + 수동 제스처, [`260806_svg_map_v6_rotation_design.md`](../260806_svg_map_v6_rotation_design.md))
...
- **추종(isFollowing)과 회전은 여전히 직교(orthogonal) 2축이다.** ◎ 버튼은 **자유↔추종 2-state** 뿐이고, 회전축은 독립적으로 움직인다 — **"자유(미추종)+회전"**(카메라는 안 따라가고 회전만 켜짐) 조합이 성립한다. 상시 follow 금지(§경로 안내 "탐색과 안내는 분리한다"와 동일 원칙)는 그대로 유지된다.
- **회전축 자체는 3-state 상태기계다(2026-08-07 개정 — 구 on/off 2-state 폐기).** `bearing` 이 이제 세 소스를 합류시킨 단일 변수다: `'north'`(0, 정방향) / `'manual'`(두 손가락 회전 제스처가 갱신하는 `manualBearing`) / `'follow'`(GPS heading 을 따르는 `compassBearing`). 하나의 변수로 합류하기 때문에 탭 히트테스트·컬링·라벨/마커 위치회전·회전 `<g>` 등 기존 6개 소비 지점은 **손대지 않아도 자동으로** 세 모드 모두를 반영한다.
  - **나침반 버튼**: 현재 `'manual'` 이거나 `'follow'` 면 `'north'`(정방향)로 복귀. 현재 `'north'` 면 `'follow'`(heading 추종)를 시작. `'manual'` → `'follow'` 직접 전이는 없다(항상 `'north'` 를 거친다).
  - **수동 두 손가락 회전 제스처**는 어느 상태에서든(`'north'`·`'follow'` 모두) `'manual'` 로 전이시킨다 — 즉 heading 추종 중에 사용자가 손으로 돌리면 추종이 해제되고 그 순간부터 수동각을 따른다.
  - 버튼 시각 상태는 새 디자인 언어를 만들지 않고 기존 `.ctrlBtnActive` 클래스(정방향=비활성/수동·추종=활성)와 `Navigation` 아이콘을 그대로 쓰되, 아이콘을 `rotate(-bearing)` 만큼 돌려 나침반 바늘처럼 북쪽을 가리키게 한다 — 수동은 고정각, 추종은 계속 움직이는 것으로 동작상 구분되고, 3가지 문구(`map.compassModeOff`/`compassModeManual`/`compassModeOn`)가 상태를 텍스트로도 구분한다.
- **수동 두 손가락 회전은 핀치줌과 같은 두 포인터 제스처 안에서 각도만 별도로 추적한다** — 거리(줌)와 각도(회전)를 배타로 만들지 않고 동시에 적용한다(일반 지도의 핀치+회전 관례). **회전 시작 데드존 = 누적 각도 6°**(`MANUAL_ROTATE_START_DEG`) — 핀치줌 도중 손끝이 자연스럽게 비틀리는 정도(실측상 흔히 2~4°)를 걸러내면서, 의도적으로 돌리기 시작하면 바로 반응하게 하기 위함이다. 데드존은 **회전 시작에만** 적용하고, 일단 회전이 시작되면 매 프레임 그대로 반영한다(진행 중 회전에 추가 데드존을 걸면 heading 데드존과 같은 이유로 반응이 끊겨 보인다). 회전 중심은 기존 `getCamCenter()`(추종/나침반과 동일)를 그대로 쓴다.
- **데스크톱 마우스 전용 회전 수단은 만들지 않았다.** 이 앱은 Capacitor WebView 기반 모바일 앱이 타깃이라 휠/드래그에 별도 modifier-key 회전 단축키를 추가하지 않았다 — 필요해지면 별도 검토 대상.
- **팬·핀치(줌)·휠 제스처는 추종만 끈다 — 회전(수동/추종 어느 쪽이든)은 그대로 둔다.** 회전은 각도이고 팬/줌은 중심·범위라 서로 충돌하지 않으며, 사용자가 명시적으로 켠 회전을 제스처가 몰래 꺼버리면 동작이 예측 불가해지기 때문이다. 유일한 예외는 수동 회전 제스처 자체(위 항목)뿐이다.
