...
- **[위치 게이트 정책 확정안 260813](260813_location_gate_policy.md)** — ⚠️ **D-1 승인 대기(권역 밖 차단화면의 Google 지도 버튼 존치 여부)**. 대표 지시 260813 11:28~11:32(*"gps 안잡히거나 지역밖이면 서비스 안해주는게 맞는거같다"*, *"경로안내 gps강제로 잡아서 시작하면 오류남 / 지도가 튄다"*) 정식화. **`service-rules.md` §폴백 정책의 개정안 — 폐기가 아니라 적용 범위 분할**: 폴백표는 *탐색형*("목록이 비어 보인다") 맥락에서 만들어졌는데 경로안내·제보에도 적용돼 있었다. **위치의 역할로 3계층** — 탐색형(조회 기준점, 홈·마켓·동네지도·정보목록)=현행 유지 / 실행형(**입력값**, 경로안내·퀘스트)=차단 / 기록형(**저장값**, 제보·피드 위치태그)=차단(폴백 저장은 데이터 위조). **P0 3건**: 카메라 튐(`origin`=폴백 vs `dotPos`=실측 불일치 + `MapControls:67-68` follow), watch 이탈판정 권역게이트 부재(시작 즉시 오류 + 유료 재탐색 2회 낭비), **침수 제보 권역검사 0**(`InfoFloodReport:42-53` — 권역 밖 좌표가 DB 저장, 복구는 어드민 수동삭제뿐). 결함 14건·확정결정 D-2~D-6·`requireServiceLocation()` 게이트 설계(판정 11곳 분산 해소)·테스트 매트릭스 ❌17칸. 태스크 SoT [`task/active/260813_location_gate_task.md`](task/active/260813_location_gate_task.md)
- **[비즈니스 광고 전자계약·결제 레일·과금 모델 ADR 260907](260907_biz_ad_payment_contract_adr.md)** — ⚠️ **대표 결정 대기(D-1~D-3) + 세무·법무 확인 4건(Q-1~Q-4)**. 한국 법인만 있어 VNPay/MoMo/ZaloPay 직가맹 불가라는 제약 아래 결정: **과금 = 월정액 tier 유지 + 선불 1·3·6개월 기간제(`paid_until`), Facebook식 예산 지갑은 Stage 3 이후로 배제** / **결제 = 계좌이체+수동승인 유지, PG 는 월 30건·주 4h 조건 충족 시 2C2P 계열 크로스보더 검토(Stripe·MoR·PayPal 배제 근거)** / **IAP = 앱 안에서 결제 완결 시 Apple 3.1.3(g) 원문상 강제 → 웹으로 뺀 현 구조 유지, 단 앱 내 "웹에서 계약하기" 버튼은 3.1.1 anti-steering(비미국 스토어프론트) 리스크라 iOS 네이티브 빌드에서 문구로 하향 권고** / **전자계약 = `checkbox_v1` 유효(전자거래법 20/2023)·벤더 보류, 대신 계약 문안 서버 SoT·동의 스냅샷·사본 다운로드 보강**. 코드 실측 결함 2건(미입금 광고 노출 게이트 누락 `ad_gating.py`, `expired` 전이·입금 기록 부재) 포함. 다음 턴 티켓 T-1~T-6(파일 후보·테스트 조건 명시). 260810 대표 결정 요청 3건(결제 위치·전자계약 방식·결제수단)에 대한 답
- **[광고 계약·입금·대조·승인 파이프라인 설계 260907](260907_ad_payment_pipeline_design.md)** — 위 ADR 의 하위 구현 설계(코드 변경 없음). **핵심 = "구현했으나 미배선" 상태를 시스템화**: 파이프라인(계약→입금→대조→승인→노출)은 전부 완성하고 배선 지점은 두 개만 — seam-A 계좌정보(`.env` `AD_PAYMENT_BANK_NAME/ACCOUNT_NO/ACCOUNT_HOLDER`, 미설정 시 계약 동의는 허용·입금 안내만 "준비 중"·기한 시계 미가동) / seam-B 입금 관측 포트 `DepositSource`(어댑터 ①manual 만 지금 구현, ②csv ③bank_feed 는 포트 수용 증명 — 은행 API 연동 = 어댑터 1파일+registry 1줄+`.env` 2키). 신설 `ad_contracts`(광고 1:N, 기간·금액 스냅샷·식별코드·동의 증거·기간) + `ad_deposits`(계약 1:N, 미매칭 NULL, `UNIQUE(source,source_ref)` 멱등) · `ad_tiers.price_3m/6m` 확정가. 식별코드 `SGR-XXXXXXC`(Crockford Base32·체크문자·정규화). 상태기계(`expired` 파생)·대조 8케이스 시스템/사람 분리·`paid_until` 이어붙이기 규칙·admin-frontend 화면 2종·Stage 1 티켓 P1-1~P1-8. 대표 결정 D-A~D-E
- **[토스페이먼츠 결제 레일 도입 설계 260907](260907_toss_payment_rail_design.md)** — ⚠️ **대표 결정 D-F~D-J(KRW 확정가 6값·콘솔취소 불허·부분환불·자동승인·계좌 병존) + 법무 L-1~L-3 + 세무 Q-2/Q-3 재검토·Q-5/Q-6**. 대표 확정(결제 레일=토스, 베트남 소비자 해외카드→한국 법인 정산)을 기존 파이프라인 위에 얹는 설계. **핵심 = 키 없는 지금도 결제 파이프라인이 끝까지 돌고, 운영 키 수령 시 코드 변경 0**(`.env` 2키 값 + 토스 콘솔 웹훅 등록만; 두 키를 읽는 파일은 `config.py` 하나 — grep 테스트로 고정). **seam-C 결제 개시 포트 `PaymentRail`**(offer/confirm/lookup/refund/parse_webhook) — 계좌이체는 null-object(RailNotSupported), 토스는 `TossGateway`(Http/Stub) 위의 어댑터; 산출물은 기존 `DepositObservation` 하나라 원장·대조·승인·노출 무변경. **토스 성공 = `ad_deposits(source=toss, source_ref=paymentKey)`** 로 confirm/웹훅 재전송 멱등, 실청구는 `charge_snapshot JSONB` 1컬럼. **카드는 자동 승인**(코어 `complete_checkout()` 이 `approve(actor=system:toss)` — 어댑터는 상태 불가침). **통화 = 표시 VND 유지 · 청구 KRW**(토스 카드결제창은 KRW 만 [확인]) — 환율 API 대신 **대표가 정하는 tier×기간 KRW 확정가 6값**(`ad_tiers.price_*_krw`, NULL=카드 버튼 숨김) 을 동의 스냅샷에 고정. 미배선 e2e 는 dev 스텁 게이트웨이(`AD_PAYMENT_TOSS_STUB=1`, APP_ENV 화이트리스트). 웹훅은 서명이 없어 `paymentKey` 재조회로 검증. P1-5/P1-6 미구현 → P2-6/P2-7 rails 버전으로 대체. 기존 두 문서 결정 대응표 §9

## 🔬 리서치

- **[업체 등록 알바 — 물품 등록 동행 포함 단가 재산정 260810](research/260810_merchant_onboarding_with_listings_pricing.md)** — 260806 PDF 제안(가입만 10건/30만동)을 **가입+매물 동행**으로 재산정. **⚠️ 선행 결정 D-1**: 업체 계정에 매물 등록 경로가 없고(`biz.py`) 매물 등록은 서버가 `phone_verified_at`을 강제해(`market.py:541-544`) 현재 코드로는 사장 개인 계정 + SMS OTP를 현장에서 받아야 하며 업체↔매물이 연결되지 않는다. **기본급 300,000동 유지 · 할당을 `가입 10` → `가입 8 + 매물 12`로 재구성**(작업 6.1시간, 분당 단가 동일 설계), 매물 초과 건당 7,000동. **8영업일 4명 = 15,582,500동(₩87만, 기존 대비 +4.6%)로 업체 256곳 + 매물 384건** — 임계 밀도 150건의 2.5배. 신규 리스크 = **건당 지급이 부르는 쓰레기 매물**(검수 6기준·업체당 5건 상한). 대표 결정 5건·투입 전 체크리스트
...
- **[사이공라이더 서비스 플로우 검토 킷 260919](review/260919_service_flow_review_kit.md)** — 대표 전달용, git 추적 대상(`ai-docs/review/`). 프레임 66개를 화면×상태 단위로 정리해 승인/수정/보류 판정을 요청하는 문서. 검토용 화면 샘플/폰 목업 스토리보드는 [`review/storyboard/index.html`](review/storyboard/index.html)(브라우저로 열람, `node ai-docs/review/storyboard/build.mjs`로 재생성)
- **[출시 준비도·당근 대비 UX·코드 수정 통합 작업서 260812](task/active/260812_launch_readiness_daangn_ux_implementation.md)** — `763bd18` pull 직후 판정·운영 실측·당근 비교·근거·실제 코드 수정·테스트·남은 RC 기준을 **이 파일 하나로 통합**. 업체 클레임·무동의 GPS·광고·공개 탐색·마켓 우선 진입·판매 초안/OTP 이탈은 코드 해결; 공급 0건, 퍼널 계측, OAuth/SMS/FCM·native·fresh DB 운영 증적이 남아 전면 공개는 NO-GO
- **[상용 서비스 여정·전환·리텐션 감사 260810](260810_service_journey_conversion_retention_audit.md)** — `a758251` 기준 화면 결함 목록과 분리해 **첫 방문→가치 인식→탐색→문의→합의→만남→완료→재방문** 퍼널을 분석. 마켓 우선 서비스 약속, 목표 여정, JCR-1~6 서비스 단절, 권장 IA, North Star·최소 이벤트·파일럿 게이트, 4개 검증 실험과 출시 시나리오를 정리. 세부 화면·기능 마찰은 아래 사용성 진단을 참조
- **[상용 서비스 사용성 진단 260810](260810_service_usability_diagnosis.md)** — **"불편해서 안 쓰게 되는 지점" 단일 SoT**. `a758251` 최신 `main` 기준 신규·구매·판매·합의·약속·이동·완료·재방문 여정을 재감사. **전면 공개 조건부 NO-GO / 한 개 구 P0 해소 후 파일럿 GO**. 콜드 스타트 악순환(가입 전 열람 차단→3km 빈 화면→판매 등록 마찰→공급 미축적), 퀘스트↔마켓 약속 충돌, 채팅·거래 단계 비가시성, 수동 약속·판매자 단독 완료, 라우팅 임시 fail-closed, 위치 오류, 제품 계측 부재를 **S-1~S-19** 코드 근거·최소 개선·결정 게이트·지표·합격 시나리오로 정리. **부록 A = 초기 매물 0 해법**(가짜 시딩 배제 원칙 / 한 개 구 집중 / 업체 매물 등록 경로·운영 대행 등록 — **둘 다 현재 코드에 없음** / "삽니다(WANTED)" 타입 신규 / 빈 결과 → 키워드 알림 CTA — **파이프라인은 이미 완비** `market.py:583-588,1016-1069` / 임계 밀도 4지표)
