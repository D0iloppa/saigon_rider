...
app.include_router(device_map.push_router)
...
# F-18: 미처리 예외 운영자 알림 (최소 경로 — sentry 대체, backend/app/main.py 미러)
@app.exception_handler(Exception)
async def unhandled_exception_alert(request, exc):
    log.exception("Unhandled exception at %s %s", request.method, request.url.path)
    await send_ops_alert(
        f"[Engine 5xx] {request.method} {request.url.path}: {type(exc).__name__}: {exc}",
        key=f"engine-exc:{type(exc).__name__}:{request.url.path}",
    )
