...
async function realFetchBlob(endpoint: string, service: Service = 'bff'): Promise<Blob> {
  const url = `${baseUrl(service)}${endpoint}`;
  const res = await fetch(url, { headers: sessionHeaders() });
  if (res.status === 419 || res.status === 401) handleSessionError();
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(extractErrorMessage(err, res.status, `GET ${url}`));
  }
  return res.blob();
}
...
export const api = { delay, realFetch, realFetchForm, realFetchBlob };
