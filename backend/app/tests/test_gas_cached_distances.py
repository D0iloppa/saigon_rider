...
from app.routers.info_gas import _with_request_distances, _without_distances, get_nearby_gas_stations, get_nearby_v2
...
class GasCachedEndpointTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_v2_cache_hit_recomputes_distance_and_order(self):
        with patch("app.routers.info_gas.cache_get", AsyncMock(return_value=self.cached)):
            response = await get_nearby_v2(10.0, 106.003, user_id=uuid.uuid4(), db=AsyncMock())

        self.assertEqual([station["station_id"] for station in response["stations"]], [2, 1])
        self.assertNotEqual(response["stations"][0]["distance_km"], 0.0)
