...
- **현재 날씨와 예보는 출처가 다르다** — 현재는 OpenWeather `/weather`(실관측), 24시간 예보는 OpenWeather `/forecast`(3시간 버킷), "1시간 내 비 확률"은 **open-meteo**(2026-08-03 "비 오는데 0%" 대응으로 추가). 한 카드에 섞어 놓으면 사용자가 모순으로 읽으므로 표시 위치로 구분한다.
