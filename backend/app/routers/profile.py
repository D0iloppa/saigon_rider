...
@router.post("/consent", response_model=UserOut, summary="약관/개인정보처리방침 동의 기록")
async def save_consent(
    body: ConsentSaveRequest, db: AsyncSession = Depends(get_db), _session_uid: uuid.UUID = Depends(verify_user_session)
):
    if body.user_id != _session_uid:
        raise HTTPException(status_code=403, detail="Forbidden")
