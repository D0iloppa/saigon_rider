...
- **네이버지도 모델(2026-08-07 2차 개정)** — 사용자 피드백("수동 회전 제스처와 핀치줌이 묶여 회전모드가 어색하다", "네이버지도처럼 평상시엔 나침반 버튼이 없고 회전한 경우에만 생겨야 한다")에 따라, heading 추종의 시작·해제를 ◎ 버튼으로 옮기고 나침반 버튼은 "회전 시에만 나타나는 북향 복귀 전용" 버튼으로 축소했다.
  ...
  - 버튼 시각 상태는 새 디자인 언어를 만들지 않고 기존 `.ctrlBtnActive` 클래스와 `LocateFixed`/`Navigation` 아이콘 선례 안에서 표현한다 — 자유는 기본 스타일+`LocateFixed`, 카메라추종은 활성 스타일+`LocateFixed`(그대로), heading추종만 아이콘을 `Navigation`(현재 bearing 만큼 회전)으로 바꿔 "지금 방향을 따라 돈다"는 것을 구분한다.
...
- **heading 소스는 자력계(DeviceOrientation) 우선, GPS course-over-ground 폴백**이다(2026-08-07 대표 지시로 도입 — "모바일 헤딩은 GPS 좌표와 무관해야 한다"). `native.watchCompassHeading()`(iOS는 `webkitCompassHeading`, Android는 `deviceorientationabsolute`→`deviceorientation` 폴백, `native.ts` 경유)이 값을 내는 동안은 그쪽을 쓰고, 자력계가 없거나 권한이 거부돼 한 번도 값이 오지 않으면 기존 GPS course 경로로 자동 폴백한다(`compassAvailableRef`). 두 소스 모두 데드존 8°(`COMPASS_DEADZONE_DEG`)를 적용해 마지막 유효 방위를 유지하며, GPS 경로는 추가로 `speed < 1.5 m/s` 이거나 `heading`/`speed` 가 `null` 이면 갱신하지 않는다(자력계 경로는 위치·속도와 무관해 이 게이트가 없다 — 정지 상태에서도 회전한다). iOS 13+ 권한 요청(`requestCompassPermission()`)은 ◎ 버튼의 카메라추종→heading추종 전이(사용자 탭) 안에서 호출한다.
