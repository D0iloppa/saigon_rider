...
export default function BizAdTierPage() {
  ...
  const openCreate = () => {
    form.setFieldsValue({ ...EMPTY_TIER, features_json: [] })
    setEditing('new')
  }

  const openEdit = (tier: BizAdTier) => {
    form.setFieldsValue({ ...tier, features_json: tier.features_json ?? [] })
    setEditing(tier)
  }
    ...
    <>
      ...
      >
        <Form form={form} layout="vertical" initialValues={EMPTY_TIER} style={{ marginTop: 20 }}>
          ...
          <Form.Item label="플랜 피처">
            <Form.List name="features_json">
              {(fields, { add, remove }) => (
                <Space direction="vertical" style={{ display: 'flex' }}>
                  {fields.map((field) => (
                    <Space key={field.key} align="baseline" style={{ display: 'flex' }}>
                      <Form.Item {...field} noStyle rules={[{ required: true, message: '피처 내용을 입력하세요.' }]}>
                        <Input placeholder="예: 홈 화면 상단 노출" style={{ width: 320 }} />
                      </Form.Item>
