...
- **동네지도 3패키지 전건 완료 (2026-07-11)** — 1차/2차/3차 코드·시각검증·독립리뷰 DONE·커밋됨.
  ...
  - **잔여**: P3 제품 결정 2건(패널 닫힘 직후 자동 말풍선 재점화 UX / view-ping 자기 포함 카운트), 소식 사진 등록 API 구현 시 상한 10 적용, codebase-memory ADR 저장(세션 MCP 재단절 — 다음 세션 필수), 운영 배포 시 init/118~122 적용+BFF 재시작+프론트 재빌드, Plane 티켓 등록(세션 내 MCP 부재).
