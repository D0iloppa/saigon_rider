...
| 2026-06-07 | F-03-IME | PROFILE-SETUP · 닉네임 한글 밀림+[시작하기] 미활성 (Android) | Android 회원가입 시 한글 한 글자씩 밀림 + 입력해도 [시작하기] 안 켜짐(공백/영문 더미 들어가야 켜짐). 블로커. 모바일Chrome·iOS·PC 정상 | **Android WebView(Capacitor) 한정** — 키보드 리사이즈 churn 이 한글 IME 조합 버퍼를 끊음. React/controlled 결함 아님(프론트 가드/uncontrolled 우회로 미해결, 사용자 4회 검증) | **네이티브: AndroidManifest windowSoftInputMode=adjustPan**(리사이즈 제거). **Android APK 재빌드 필요**. android `2b7ce8d`. 프론트 보조: Android 한정 uncontrolled+input 리스너. iOS 무관 | 🔧 빌드대기 |
...
| 2026-06-07 | F-03-KBD | PROFILE-SETUP · 키보드 시 레이아웃 압축 (Android) | Android 키보드 표시 시 컴포넌트 영역 압축. 아이폰 정상 | softInputMode 미지정 → OS 기본 리사이즈 + AppShell `100dvh` 축소 | **네이티브 AndroidManifest windowSoftInputMode=adjustPan**(리사이즈→pan). Android APK 재빌드 필요. android `2b7ce8d`. (런타임 interactive-widget 주입은 Blink 가 파싱 후 변경 무시 → 무효, 정적 meta 로 이전했으나 네이티브가 본 해결) | 🔧 빌드대기 |
