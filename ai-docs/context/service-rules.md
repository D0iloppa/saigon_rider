...
4. **예외 — 홈 헤더 위치 행 (사용자 지시 2026-08-03).** 홈(`WorldMapV2`) 진입 시 **표시 목적으로만** GPS 1회를 측정해(`native.ensureLocationPermission()` → `native.getLocation()`) 현재 동 이름을 헤더에 보여준다(`home.v2.currentRegion`). 원칙 1·2 의 명시적 예외이며 다음 제약을 지킨다:
   - **표시 전용이다.** 목록 조회 기준 좌표(`coords`/`resolvedWard`)와 `useLocationStore` 는 **건드리지 않는다** — SoT 는 여전히 사용자가 고른 지역이다.
   - **실패하면 조용히 폴백한다.** 권한 거부·타임아웃·측정 실패 시 기존 `home.v2.defaultRegion` 표기를 그대로 쓰고, 홈 진입을 막거나 재요청하지 않는다.
   - **HCMC 밖이면 폴백한다.** 판정은 `wardRegionAt()` 폴리곤(중심부 37개 동)으로 하며, `null` 이면 기본 지역 표기로 되돌아간다. ⚠️ 이 폴리곤은 HCMC 전역이 아니라 중심부만 커버하므로 **Thủ Đức·Bình Tân·Gò Vấp 등에 있어도 폴백**된다 — 데이터 한계이지 버그가 아니다.
   - 이 예외를 다른 화면으로 확산하지 말 것. 지도·정보 화면은 원칙 2 가 그대로 적용된다.
5. **예외 — 지도의 '내 위치' 파란 점 (사용자 지시 2026-08-04, 실시간 추종 2026-08-05).** 지도 화면(동네지도·마켓지도)은 진입 시 GPS 1회를 **점 표시 목적으로만** 읽고(`SaigonMapV5` `meDotOnMount`), **점이 찍힌 뒤에는 `native.watchLocation` 으로 점 좌표를 실시간 갱신한다**(대표 지적: "표시는 하는데 실시간 반영을 안 한다"). 원칙 1·2 의 예외이며 제약은 홈 헤더(예외 4)와 같은 계열이다:
   - **watch 는 점이 이미 찍힌 화면에서만 돈다.** 점을 쓰지 않는 화면(위치 피커 등)에서는 GPS 를 켜지 않으며, 언마운트/점 소멸 시 해제한다. 서비스 지역 밖 좌표는 갱신을 건너뛴다(마지막 유효 위치 유지).
   - **watch 도 카메라를 따라가지 않는다** — 점만 움직인다. 자동 recenter 를 붙이지 말 것(지도 조작을 빼앗는다).
   - **표시 전용이다.** 카메라(뷰포트)·선택 동(`selWard`)·`useLocationStore` 를 건드리지 않는다 — 지역 탐색 SoT 는 여전히 사용자가 고른 지역이다. 카메라까지 옮기는 `locateOnMount` 와는 다른 prop 이니 혼용하지 말 것(`locateOnMount` 를 지역 선택 상태에서 켜면 선택 경계와 카메라가 어긋나는 회귀가 재발한다).
   - **마운트 1회로 고정.** 지역칩 ✕ 해제 등으로 조건이 뒤집혀도 재측위하지 않는다.
   - **실패·서비스 지역 밖이면 점을 찍지 않는다.** 토스트도 띄우지 않는다(장식용 표시 — 안내는 위치 기능을 명시적으로 누른 호출부 책임).
