...
- **수동 두 손가락 회전은 핀치줌과 같은 두 포인터 제스처 안에서 각도만 별도로 추적한다** — 거리(줌)와 각도(회전)를 배타로 만들지 않고 동시에 적용한다(일반 지도의 핀치+회전 관례). 다만 대표 피드백("줌만 하려는데 손가락이 틀어지면 회전이 걸림")에 따라 회전 판정을 두 겹으로 강화했다(2026-08-07 2차):
  - **회전 시작 데드존 = 누적 각도 10°**(`MANUAL_ROTATE_START_DEG`, 기존 6°에서 상향) — 순수 줌 의도 핀치에서도 손끝이 비대칭으로 움직이면 6~8°까지 각도가 누적될 수 있어, 의도치 않은 회전 유발 사례가 있었다.
  - **지배성 판정**(`ROTATE_DOMINANCE_RATIO = 1.2`) — 회전이 만든 호 길이(반지름×누적각도 라디안, px)가 줌이 만든 누적 거리변화(px)의 1.2배를 넘어야만 각도 데드존과 별개로 회전으로 커밋한다. 순수 줌 제스처는 거리변화가 지배적이라 데드존을 넘어도 커밋되지 않는다.
