# Saigon Rider — Claude Code 작업 지시서 v1.0

> 발행일: 2026-05-18
> 형식: Claude Code (`claude` CLI)에 그대로 던지면 작동
> 범위: Phase 0 (모노레포 + 토큰 + 컴포넌트) + Phase 1 (인증 + 홈)
> 전제: D/C/B 결정서를 다 읽었거나, 또는 이 문서를 단독으로 실행 가능
...
- B-design-tokens-components.md (토큰/컴포넌트)
- sre-* 백엔드 SQL 7개 (마이그레이션용)

위 파일들은 /docs 폴더에 넣을 거야. 나는 위임형이니까 적극적으로 결정하고
진행해. 막힌 게 있으면 구체적으로 질문해.
...
## §1. Phase 0 — 모노레포 + 디자인 시스템 (예상 2-3일)
...
**Day 1**: Task 1+2+3 (모노레포 + tokens + ui 5개)
**Day 2**: Task 4 (Storybook) + Task 5 (Expo)
**Day 3**: Task 6+7 (Supabase 마이그레이션 + api 패키지)
**Day 4**: Task 8 (Expo Router) + Task 9.1+9.2 (SPLASH + PHONE)
**Day 5**: Task 9.3+9.4 (OTP + PROFILE-SETUP)
**Day 6**: Task 10 Step A (HOME 기본)
**Day 7**: Task 10 Step B (HOME 디자인) + 폴리시
**Day 8-10**: 버그픽스, OnTimePush 권한, EAS Preview 빌드 첫 시도
