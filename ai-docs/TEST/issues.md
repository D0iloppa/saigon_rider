...
| 2026-06-07 | F-03-IME | (전역) 모든 입력창 한글 밀림 (Android) | Android 앱 **모든 입력창**에서 한글이 한 칸씩 밀림(다음 글자 쳐야 앞글자 커밋). 블로커. iOS·PC·모바일Chrome 정상 | **확정**: Capacitor `captureInput=true` → `CapacitorWebView.onCreateInputConnection` 이 조합 미지원 `BaseInputConnection(this,false)` 반환 → 전역 IME 조합 깨짐. React/controlled 무관(프론트 우회 전부 무효였던 이유). **모바일 Chrome 정상=앱 WebView 설정 문제로 격리** | **captureInput=false** — MainActivity CapConfig **+ `assets/capacitor.config.json`(WebView 실제 로드 파일, cap sync 미실행으로 true 잔존했던 게 핵심)** + capacitor.config.ts. **클린 Android 재빌드**(android `1d5a01f`). **✅ 2026-06-08 해결 확인** | ✅ |
...
| 2026-06-07 | F-03-KBD | PROFILE-SETUP · 닉네임 입력창/카드 높이 줄어듦 (Android) | Android 키보드 표시 시 입력창·카드 높이 압축. 아이폰 정상 | 뷰포트 축소 시 `.nickField`(60px)·`.styleCard` 가 flex 자식 기본 `flex-shrink:1` 로 눌림(키보드 모드 무관 CSS) | **`flex-shrink:0`** 부여(ProfileSetup.module.css) → 압축 대신 body 스크롤. **web 수정**. 보조 native adjustPan. **✅ 2026-06-08 해결 확인** | ✅ |
| 2026-08-06 | 마일리지 거리 게이트 무력화 (머니 경로) | Engine `mileage.py::_apply_event_time_policy` | 대표 지적 — 누적 주행거리 **1,075km** 표시("난 집에만 있어"). `@capacitor/geolocation` 이 포그라운드 전용이라 앱을 닫아둔 공백(7.2h) 뒤 재실행 시 그 사이 좌표 점프가 한 덩어리 거리로 들어와 "50km/h 정상 주행"으로 통과. `total_distance_m` 은 마일리지 정책 반복 달성 조건이라 RP 적립→쿠폰·기프티콘 환금까지 이어지는 머니 경로 결함 | **확정**: 속도 게이트(3~150km/h)만으로는 통과 가능 최대 거리가 `150km/h × Δt` 로 공백 길이에 정비례해 무력화됨. 추가로 `previous_at is None`(첫 이벤트)은 속도 검사 자체를 건너뛰어 상한 없음 | `MAX_GAP_S=300`(5분 초과 공백은 거리 미계상) + `MAX_EVENT_DISTANCE_M=12_500`(단일 이벤트 절대 상한, `previous_at` 유무 무관) 게이트 추가. `ordered` 의미는 유지(거리만 0). 테스트 9건(`test_gps_pipeline.py`) 추가, 결함 재현(T-1·T-6) 수정 전 실패 확인 후 수정. 커밋 `e1e787a`. 오염 데이터 정정(M-2)·계정 부활(M-3)·UI 표시 제거(M-1)는 대표 결정 대기, 이번 수정 범위 아님. **codebase-memory 재인덱싱은 MCP 세션 중 끊겨 미실행 — 다음 세션 처리 필요** | ✅ |
