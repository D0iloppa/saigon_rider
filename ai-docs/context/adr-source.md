...
- **폴링 실태(2026-09-13 전수조사)**: 홈 유휴 시 도는 폴러는 뱃지 1개뿐이고 1분당 약 4건(10초로 낮춘 뒤 12건) — 부하는 문제가 아니었다. 실제 낭비는 백그라운드에서도 도는 폴러들이었고 `pollScheduler` 로 정리했다. 스케줄러를 **안 쓰는 예외는 4개**다. `RideNav` trail·`useRideStore` active-card(3s)는 길안내·퀘스트 추적이라 화면이 꺼져도 돌아야 해서 **의도적으로** 제외했고, `WalkieTalkieFloatingButton` presence(15s)·`useBizViewerCount`(15s)는 이미 직접 가드를 갖고 있어 아직 이관하지 않았다(동작은 옳다 — 이관은 정리 차원의 후속).
