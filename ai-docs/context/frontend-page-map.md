...
| 라우트(SPA) | 페이지 파일 | 내용 |
|---|---|---|
| `/sre/reward-policies` | `admin-frontend/src/pages/sre/RewardPolicyPage.tsx` | **보상 정책 (2026-07-23 이식, 커밋 `f32cc36`, opus 리뷰 PASS)** — reward_policy CRUD(policy_code·conditions[]·actions[] GRANT_XP/EXP/GOLD/BADGE). 백엔드 `admin_api/reward_policy.py` 5라우트 **전부 `verify_root_api`** → `engine_client.admin_*_policy` → Engine `/v1/admin/policies*` 1:1 프록시. 감사 `REWARD_POLICY_*`. **⚠️ 기존 Engine 버그**: `engine/app/routers/admin.py:705` `_policy_to_dict`가 refresh 안 된 action 행에 `.value` → actions 포함 create/update/toggle 시 Engine 500→502(**DB 쓰기는 성공하나 응답 실패, 레거시도 동일**). 대표 레인 별도 티켓(수정: `db.refresh` 또는 enum 강제). 그 버그로 CREATE/UPDATE 감사로그도 누락됨(DELETE·list·get은 정상) |
| `/sre/items` | `admin-frontend/src/pages/sre/ItemListPage.tsx` | **아이템 관리 (2026-07-23 이식, 커밋 `1c72456`)** — item_definition CRUD(item_code·display_name·slot(23종)·rarity(C/R/E/L/M)·collection_code·asset_uri·shop_price_gp/gc·is_shop_visible·season_lock·effect_type). 백엔드 `admin_api/items.py` 5라우트 **전부 `verify_root_api`** → `engine_client.admin_*_item` → Engine `/v1/admin/items*`(_svc) 1:1 프록시. 감사 `ITEM_*`. 가챠/상점 게이트OFF라 라이브 영향 낮음. **⚠️ 기존 DB 스키마 갭**: `item_definition.collection_code`가 DB는 NOT-NULL FK인데 API/레거시폼은 nullable 취급 → 빈/미존재 code로 create 시 500(레거시 동일). 대표 레인 |
| `/sre/ops` | `admin-frontend/src/pages/sre/OpsDashboardPage.tsx` | **SRE 운영 대시보드 (2026-07-23 이식, 커밋 `d95bed4`)** — 읽기전용 경제 모니터링 4지표: 일별 GOLD/XP earn/spend/net(7일)·가챠 ROI(30일)·가챠vs상점 채널비율·pity 분포. 백엔드 `admin_api/ops.py` 4 GET **전부 `verify_root_api`** → `engine_client.admin_ops_*` → Engine `/v1/admin/ops/*`(_svc). 감사 없음(read-only). net 색상·dup율≥60% 강조 레거시 동일. **미이식 SRE 크로스링크**: `/sre/gacha`·`/sre/shop`·`/sre/daily-featured`(게이미피케이션 게이트OFF, 저우선 잔여) |
...
| `/sre/quests` | `admin-frontend/src/pages/sre/QuestListPage.tsx` | **퀘스트 관리 (2026-07-23 이식, 커밋 `0cbc707`)** — Quest CRUD, 필드 title_ko/vi/en·period(DAILY/WEEKLY/EVENT)·district_id(FK)·required_level·target_distance_km·badge(HOT/NEW/LIMITED)·is_active·reward_exp/gold/item·starts_at/ends_at·이미지3슬롯(main/thumb/banner content_id, imgproxy). 백엔드 `admin_api/quests.py` **전부 `verify_root_api`**(RP지급 연관) + `GET /quests/meta`(district/period/badge 드롭다운). 감사 `QUEST_*`. BFF-로컬(Engine 무관). 레거시 폼과 동일 필드셋(criteria JSONB·rider_type 등은 레거시도 미노출이라 제외). **⚠️ 기존 버그(대표)**: `quests.card_type` DB=native enum vs ORM=String(20) → INSERT(레거시·신규 공통) 500 → **퀘스트 생성 불능**(수정: models.py 타입 or INSERT 캐스트) |
