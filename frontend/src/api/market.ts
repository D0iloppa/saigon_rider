...
export interface ListingDetail {
  ...
  imageUrls: string[];
  /** image_urls 와 순서 대응 — 수정 화면에서 기존 사진을 재업로드 없이 유지하는 데 사용 */
...
export async function withdrawListing(id: string, sellerId: string): Promise<{ id: string }> {
  return api.realFetch<{ id: string }>(`/market/listings/${id}/status`, {
    method: 'PATCH',
    body: JSON.stringify({ seller_id: sellerId, status: 'WITHDRAWN' }),
  }, 'bff', { rethrow: true });
}
...
export async function reportListing(
  ...
  await api.realFetch(
    ...
    {
      method: 'POST',
      body: JSON.stringify({ reason, note: note ?? null, image_content_ids: imageContentIds ?? [] }),
    },
    'bff',
    { rethrow: true },
  );
...
export async function updateListing(id: string, p: UpdateListingParams): Promise<{ id: string }> {
  return api.realFetch<{ id: string }>(`/market/listings/${id}`, {
    method: 'PATCH',
    body: JSON.stringify({
...
export async function fetchListing(id: string, userId?: string): Promise<ListingDetail> {
  ...
  return {
    ...
    imageContentIds: r.image_content_ids ?? [],
    seller: {
      id: r.seller.id,
