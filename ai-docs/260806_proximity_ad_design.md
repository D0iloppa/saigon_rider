...
> 상태: **범위 A 착수 확정 (2026-08-06)** — D-1/D-2/D-6 확정, D-3/D-4/D-5 미결. §9 백엔드 구현(1~6)·프론트 진입감지(7)은 **미착수**(오픈 범위 밖, §7)
> 관련: [`spec/ad-performance-metrics.md`](spec/ad-performance-metrics.md) · [`context/architecture.md`](context/architecture.md) · [`context/service-rules.md`](context/service-rules.md)
...
CREATE TABLE IF NOT EXISTS proximity_policy (
    ...
    daily_notify_cap        INTEGER NOT NULL DEFAULT 5,     -- 사용자별 일일 알림 상한
...
1. **8월 오픈**: GPS 기본 통일 + 카메라 추적(`follow`·나침반) — **UI 계층만.** 이미 `MapCanvas.follow()` 존재, 마켓·동네지도 확장만.
2. **오픈 직후**: `proximity_policy.is_enabled=FALSE` 로 스키마·모듈 선반영 (무동작)
3. **가맹점 확보 후**(알바 320건 등록 완료 시점): 킬스위치 ON + 소규모 검증

가맹점이 없으면 근접 광고는 노출할 대상이 없다. **선행조건은 코드가 아니라 가맹점 수다.**

---

## 8. 착수 전 필요한 결정 (대표 승인 항목)

| # | 결정 항목 | 권고안 | 결정 (2026-08-06) |
|---|---|---|---|
| D-1 | 백그라운드 위치 추적 도입 여부 | **미도입** (포그라운드 전용) — R-1 | **확정: 미도입** |
| D-2 | 방문 확정 방식 | GPS 체류 + **QR 스캔 병행 검토** — R-2 | **확정: GPS 체류 + 속도 일관성. QR 스캔은 v2** |
| D-3 | 알림 반경 / 방문 반경 | 300m / 50m (조정 가능, 테이블화됨) | 미결 |
| D-4 | 일일 알림 상한 | 5건 — R-3 | 미결 |
| D-5 | 포인트 적립량 | `action_definition.rp_grant` 값 — 미정 | 미결 |
| D-6 | 오픈 범위 포함 여부 | **제외.** §7 | **확정: 오픈 범위 제외** |
...
6. engine_client → BIZ_PROXIMITY_VISIT RP 적립
