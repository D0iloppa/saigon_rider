...
import { ArrowLeft, Bike, ChevronRight, Sparkles, User, type LucideIcon } from 'lucide-react';
...
interface TabDef {
  i18nKey: string;
  /** 카테고리 탭 UI 아이콘 (lucide) — 슬롯 이모지(게임 콘텐츠)와 별개 */
  icon: LucideIcon;
  silhouette: string;
  left: SlotDef[];
  right: SlotDef[];
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.header}>
        <button className={s.headerBack} onClick={() => navigate(-1)} aria-label={t('common.back')}>
      ...
      <div className={s.catTabs}>
        {TAB_KEYS.map((key) => {
          ...
          return (
            ...
            >
              <TabIcon size={14} strokeWidth={2} aria-hidden /> {t(TABS[key].i18nKey)}
            </button>
          );
        })}
      </div>
        ...
        <>
          ...
          {effectTotals.length > 0 ? (
            <button className={s.statsBar} onClick={() => setEffectSheetOpen(true)}>
              ...
              <span className={s.effectMore} aria-hidden>
