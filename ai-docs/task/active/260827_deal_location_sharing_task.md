...
>
> **상태**: 설계 확정, §10 미결사항 전건 확정(2026-08-27) — **구현 착수 가능.**
>
...
## 10. 대표 판단이 필요한 미결 사항 — **확정 완료 (2026-08-27)**

| # | 항목 | 확정 |
|---|---|---|
| M-1 | §3 정밀도 매트릭스 확정 | **표 그대로 승인** |
| M-2 | `exact` 창 폭 | **T-30분 / T+60분** |
| M-3 | `COMPLETED` 후 약속 핀 잔존 | **`approx` 유지**(거래이력 열람용, 실시간좌표는 별도로 즉시삭제) |
| M-4 | 백그라운드 위치 추적 허용 여부 | **포그라운드 한정** |
| M-5 | 배터리 정책 | **10초 + 10m** |
| M-6 | 실시간 공유 대칭 강제 | **한쪽만도 허용**(UI에 상대 상태 명시) |
| M-7 | 동의 UX | **거래(약속)마다** |
| M-8 | 실시간 좌표 보관 | **최신 1건만**(궤적 미보관) |
| M-9 | 워키토키와 동의 통합 여부 | **기능별 각각** |
