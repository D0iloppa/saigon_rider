...
> **개정 이력 (2026-08-06, 4차)**: D-C(나침반 모드 L3 비활성) 폐기 — 회전 중에도 L3·POI 표시(사용자 지시). §10 알려진 갭 1(쿼리 bbox 미회전) 해소 — `onBboxChange` 를 `rotatedBBoxOfRect` 로 확장. §2.5 제스처 역회전 4곳 중 실측 결과 탭 1곳만 필요함을 확인, 휠/핀치중심/팬 3곳의 +bearing 보정은 결함이라 제거(08cd1e3 정정).
