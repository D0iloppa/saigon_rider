...
export interface UserDto {
  ...
  manner_temp?: number;
  created_at: string;
  // F-9: null 이면 약관/개인정보처리방침 동의 미기록 — 필드 자체가 없으면(구버전 응답 등)
  // 판정 불가로 간주해 막지 않는다(undefined 는 차단하지 않음, null 만 차단).
...
export async function apiDevLoginAs(userId: string): Promise<OAuthLoginResult> {
  return api.realFetch<OAuthLoginResult>('/auth/dev-login-as', {
    method: 'POST',
