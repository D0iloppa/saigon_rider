...
14. **일회성 측위와 지속 추적은 폴백 대칭이어야 한다** (2026-08-07 실기기 결함). `@capacitor/geolocation` 이 두 플랫폼에서 배제돼 있으므로 `getLocation()`·`watchLocation()` **둘 다** `navigator.geolocation` 폴백을 가져야 한다. 한쪽만 있으면 실기기에서 **일회성은 되는데 추적만 죽어** 카메라 연출·나침반·이탈·도착이 전부 조용히 사라진다. `Geolocation.watchPosition` 은 미등록 플러그인 호출을 에러로 reject 하지 않고 watch id 만 돌려준 채 콜백을 영원히 안 부르는 **침묵 실패**를 낼 수 있어, 에러 핸들러로는 못 잡는다 — 첫 틱이 일정 시간(`native.ts` 의 `FALLBACK_TIMEOUT_MS=7초`) 안에 안 들어오면 `navigator.geolocation.watchPosition` 으로 전환한다. **폴백 전환 시에도 워처는 여전히 1개**여야 한다(원칙 7) — 전환 직전 기존 capacitor 구독을 반드시 정리해 두 소스가 동시에 콜백을 때리지 않게 한다.
...
### 회전(나침반) — `enableFollowCompass` (제정 2026-08-06, 개정 2026-08-06 — 추종/나침반 직교 2축, [`260806_svg_map_v6_rotation_design.md`](../260806_svg_map_v6_rotation_design.md))
...
- **추종과 나침반은 직교(orthogonal) 2축이다(구 3-state 순환 폐기).** ◎ 버튼은 **자유↔추종 2-state** 뿐이고, 나침반은 **별도 토글 버튼**으로 독립적으로 켜고 끈다 — **"자유(미추종)+나침반"**(카메라는 안 따라가고 회전만 켜짐) 조합이 성립한다. 상시 follow 금지(§경로 안내 "탐색과 안내는 분리한다"와 동일 원칙)는 그대로 유지된다.
- **팬·핀치·휠 제스처는 추종만 끈다 — 나침반은 그대로 둔다.** 회전은 각도이고 팬/줌은 중심·범위라 서로 충돌하지 않으며, 사용자가 명시적으로 켠 나침반 토글을 제스처가 몰래 꺼버리면 동작이 예측 불가해지기 때문이다.
