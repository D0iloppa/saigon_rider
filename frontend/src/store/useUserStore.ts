...
interface UserState {
  ...
  markConsentAgreed: (agreedAt: string) => void;
  logout: () => void;
...
function dtoToUser(dto: UserDto): User {
  ...
  return {
    ...
    consentAgreedAt: dto.consent_agreed_at,
  };
}
...
export const useUserStore = create<UserState>()(
  persist(
    (set, get) => ({
      ...
      // F-9: 로컬 상태만으로 게이트를 통과시키면 재설치·다른 기기에서 우회되므로,
      // 서버가 실제로 기록에 성공했을 때(apiSaveConsent 응답)만 호출한다.
      markConsentAgreed: (agreedAt) => {
        const u = get().user;
        if (!u) return;
        set({ user: { ...u, consentAgreedAt: agreedAt } });
      },
