...
def _issue_to_todo(issue: dict) -> dict:
    ...
    return {
        ...
        "status": feat["status"],
        "feature_id": None,
        "feature": {"id": feat["id"], "category": feat["category"], "name": feat["name"],
                     "description": None, "status": feat["status"], "sort_order": 0,
                     "meta": None, "created_at": feat["created_at"], "updated_at": feat["updated_at"]}
                    if feat["category"] else None,
...
async def list_todos(
    ...
    todos.sort(key=lambda t: (t["status"], -(["LOW", "MEDIUM", "HIGH", "URGENT"].index(t["priority"])
                                              if t["priority"] in ["LOW", "MEDIUM", "HIGH", "URGENT"] else 0)))
    total = len(todos)
    start = (page - 1) * size
