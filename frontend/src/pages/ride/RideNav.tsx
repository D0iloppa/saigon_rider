...
import {
  ArrowUp, ChevronLeft, Compass, CornerUpLeft, CornerUpRight,
  Flag, Hourglass, Navigation2, Play, RefreshCw, Undo2,
} from 'lucide-react';
...
function ManeuverIcon({ maneuver, isLast = false }: { maneuver?: string | null; isLast?: boolean }) {
  const m = maneuver ?? '';
  const Ico = isLast ? Flag
    : m.includes('uturn') ? Undo2
    : m.includes('roundabout') || m.includes('rotary') ? RefreshCw
    : m.includes('left') ? CornerUpLeft
    : m.includes('right') ? CornerUpRight
    : ArrowUp;
  return <Ico size={18} strokeWidth={2.2} aria-hidden="true" />;
}
...
export default function RideNav() {
  ...
  return (
    <div className={styles.page}>
      <button className={styles.backBtn} onClick={handleBack} aria-label={t('common.back', '뒤로')}>
        ...
        <>
          ...
          {type === 'nav' && guidanceStarted && compass && (
            <div className={styles.compassChip}>
              ...
              >
                <Navigation2 size={18} fill="currentColor" strokeWidth={0} aria-hidden="true" />
              </span>
              ...
              type === 'nav' ? (
                <div className={styles.etaRow}>
                  ...
                  <div className={styles.etaDist}>
                    <div className={`${styles.distVal} num`}>{route?.distance_text ?? '—'}</div>
                ...
                <div className={styles.etaRow}>
                  ...
                  <div className={styles.etaDist}>
                    <div className={`${styles.distVal} num`}>
                      {mode === 'checkpoint'
                      ...
                      return (
                        <div className={styles.stepRow} key={i}>
                          <div className={`${styles.stepIcon} ${last ? styles.stepIconEnd : ''}`}>
                            <ManeuverIcon maneuver={s.maneuver} isLast={last} />
                          </div>
              ...
              <div className={styles.questBody}>
                {mode === 'checkpoint' ? (
                  <>
                    <div className={styles.statRow}>
                      <div className={styles.statCell}>
                        <div className={`${styles.statVal} num`}>
                          {checkpointDistM != null ? formatDistance(checkpointDistM) : '—'}
                        </div>
                        <div className={styles.statLabel}>{t('rideNav.remaining', '남은 거리')}</div>
                      </div>
                      <div className={styles.statCell}>
                        <div className={`${styles.statVal} num`}>{formatDuration(ride.durationSec)}</div>
                  ...
                  <>
                    ...
                    <div className={styles.statRow}>
                      <div className={styles.statCell}>
                        <div className={`${styles.statVal} num`}>{formatDistance(ride.distanceM)}</div>
