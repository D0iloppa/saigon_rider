...
import {
  useRepairSubmissions,
  useConfirmRepairSubmission,
  useRejectRepairSubmission,
  type RepairSubmissionRow,
} from '../../api/map'
...
export default function RepairSubmissionListPage() {
  const [status, setStatus] = useState('PENDING')
  const [rejectTarget, setRejectTarget] = useState<RepairSubmissionRow | null>(null)
  const [reviewNote, setReviewNote] = useState('')

  const { data, isLoading, isError, error } = useRepairSubmissions(status || undefined)
  const confirmMutation = useConfirmRepairSubmission()
  const rejectMutation = useRejectRepairSubmission()
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'actions',
      width: 160,
      render: (_: unknown, r: RepairSubmissionRow) =>
        r.status === 'PENDING' ? (
          <Space>
            <Popconfirm
              title="이 정비소 제보를 승인하시겠습니까? (정비소로 등록됩니다)"
              onConfirm={() =>
  ...
  return (
    <>
      ...
      <Table<RepairSubmissionRow>
        rowKey="submission_id"
        loading={isLoading}
        columns={columns}
        dataSource={data ?? []}
        locale={{ emptyText: '등록된 정비소 제보가 없습니다' }}
        pagination={false}
      />
      <Modal
        title="정비소 제보 반려"
