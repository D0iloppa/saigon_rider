...
| 키 | dev | **prod** |
|---|---|---|
| `BFF_PUBLIC_URL` | `https://saigon.doil.me/api/bff` (dev 실제값, **`/api/bff` 포함**) | **`https://letantonsheriff.com/api/bff`** |
...
- ⚠️ **ENGINE_SERVICE_KEY 정렬(06-04, 회전 철회)**: 네이티브 앱이 `serviceKey="dev_engine_service_key_change_in_prod"`를 단일로 박아 SRE(백그라운드 GPS)에 사용 → 운영 engine도 그 값이어야 인증. 그래서 운영 `.env`의 `ENGINE_SERVICE_KEY`를 회전값에서 **앱 값으로 정렬**(사용자 결정). 검증: 올바른키 200·틀린키/무키 401. **보안 트레이드오프**: 이 키는 bff→engine 내부 인증도 겸하는데 앱에 박혀 추출가능 → 사실상 반공개. 정석은 SRE 전용 키 분리(engine 변경, 후속).
