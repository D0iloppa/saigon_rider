...
export default function QuestList() {
  ...
  const { items: quests, isLoading: loading, isLoadingMore, hasMore, error: questsError, sentinelRef, reset } =
  ...
  return (
    <div className={styles.root}>
      ...
      <div className={styles.header}>
        ...
        {mode === 'catalog' && <div className={styles.filterRow}>
          {districts.map((d) => (
            ...
            >
              <Building2 size={12} strokeWidth={2.5} aria-hidden /> {localizedName(d)}
            </Chip>
          ))}
        </div>}
      ...
      <div className={styles.listContent} style={contentStyle}>
        ...
        ) : quests.length === 0 && questsError ? (
          // F-12: 조회 실패를 "퀘스트 없음"으로 위장하지 않고 구분해 재시도를 제공
          <div className={styles.empty}>
            <StateBlock
              icon={AlertCircle}
              tone="error"
              title={t('quest.loadError', { defaultValue: '퀘스트를 불러오지 못했어요' })}
